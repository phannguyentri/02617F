   ...
   <div class="modal-content">
      <div class="modal-header">
         ...
         <h4 class="modal-title" id="myModalLabel" style="text-transform: uppercase;">
            Danh sách công việc thuộc giao dịch <?php echo $task->name ?>
         </h4>

      </div>
      <div class="modal-body">
         <div class="col-xs-12" style="padding: 0px; padding-bottom: 15px;">
<!--                 <?php if(has_permission('tasks','','create')){ ?>
                <a href="#" onclick="new_task_from_relation('<?php echo $task->id ?>','<?php echo $task->rel_id ?>','<?php echo $task->rel_type ?>'); return false;" class="btn btn-info pull-left new"><?php echo _l('Công việc mới'); ?></a>
                <?php } ?> -->
                <?php if(has_permission('tasks','','create')){ ?>
                  <span class="btn btn-info" id="createNewTask">Công việc mới</span>
                <?php } ?>
                <span class="btn btn-danger pull-right" id="searchav">Tìm kiếm nâng cao</span>


         </div>

         <div class="col-xs-12 " data-id="searchav" style="display: none;padding: 15px 0px 20px 0px;margin-bottom: 15px;  border:  1px solid #ccc">
           <div class="col-xs-4 col-sm-4 col-md-4 col-lg-4">
                        ...
                        echo render_select('purpose1', $purpose_type, array('id','name'),'Mục đích','', array(), array(), '', '', false);
                        ?>
                       </div>
                       <div class="col-xs-4 col-sm-4 col-md-4 col-lg-4">
                          <?php
                        ...
                        echo render_select('transaction1', $transaction_type, array('id','name'),'Phương thức', '', array(), array(), '', '', false);
                        ?>
                       </div>
                        ...
                        $remi_type = array(
                          array(
                            ...
                            array(
                                'id' => '2',
                                'name' => 'Chưa tạo nhắc nhở',
                            ),
                            array(
                                'id' => '1',
                                'name' => 'Đã tạo nhắc nhở',
                            ),


                        );

                        echo render_select('remi', $remi_type, array('id','name'),'Nhắc nhở', '', array(), array(), '', '', false);
                        ?>
                       </div>
         ...
         <div class="col-xs-12 " data-id="createNewTask" style="display: none;padding: 15px 0px 20px 0px;margin-bottom: 15px;  border:  1px solid #ccc">
          ...
          <div class="col-md-12">
             ...
             <?php
                ...
                ?>
             <div class="checkbox checkbox-primary no-mtop checkbox-inline hidden">
             ...
             <div class="checkbox checkbox-primary checkbox-inline hidden">
                <input type="checkbox" id="task_is_billable" name="billable" <?php if(isset($task)){if($task->billable == 1){echo 'checked';}}; ?>>
             ...
             <!-- <hr /> -->
             <?php echo render_input('name','Tên công việc', ''); ?>
...
<!--                 <div class="col-md-6 hidden">
                ...
                <input type="hidden" name="taks1_id" value="<?php echo $id_task ?>">
                 <div class="col-md-6 hidden">
             ...
             <div id="recurring_ends_on " class="hidden <?php if(!isset($task) || (isset($task) && $task->recurring == 0)){echo 'hide';}?>">
             ...
             <p class="bold"><?php echo _l('Nội dung chốt cuối cùng'); ?></p>
             <?php $contents1 = ''; if(isset($task)){$contents = $task->description1;} ?>
             <?php echo render_textarea('description1','',$contents1,array('data-task-ae-editor'=>true),array(),'','tinymce-task'); ?>


             <button id="btn-create-new-task" type="submit" class="btn btn-info">Lưu</button>
          </div>
...
<script>
  ...
  initDataTable('.table-tasks', admin_url + 'tasks/work/' + id, [1], [1],filterList);
  ...
  $('#btn-create-new-task').click(function(){
    ...
    let rel_id              = <?php echo($task->rel_id) ?>;
    let rel_type            = '<?php echo($task->rel_type) ?>';
    let priority            = <?php echo($task->priority) ?>;
    let datefinished        = '<?php echo($task->datefinished) ?>';
    let repeat_every        = $('#repeat_every').val();
    let repeat_type_custom  = $('#repeat_type_custom').val();



    $.ajax({
      url: admin_url+'/Tasks/addNewTask',
      type: 'get',
      dataType: 'json',
      data: {
        name                : name,
        ...
        repeat_type_custom  : repeat_type_custom
      },
    })
    .done(function(responses) {
      console.log("success");
      console.log(responses);
      $('div[data-id="createNewTask"]').slideToggle();
   ...
   <?php if(isset($_milestone_selected_data)){ ?>
    ...
    $('#searchav').click(function(){
      $('div[data-id="searchav"]').slideToggle();
    })
