...
class Home extends Admin_controller
{
    ...
    public function index()
    {
        ...
        {
            // echo $_SESSION['type_role'];
            $data['title']="Chọn phòng ban";
             $this->load->view('admin/setup_home', $data);
        }
    ...
    public function check_type_role($type)
    {
        if($type)
        {
            if($type==1)
            {
                $this->session->set_userdata('type_role','setup_menu_active_sales');
            }
            else
                if($type==2)
                {
                    $this->session->set_userdata('type_role','aside_menu_active');
                }
        }
        // echo $_SESSION['type_role'];
        $this->index();
